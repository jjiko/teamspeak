...
namespace Jiko\Teamspeak\Com\Bots;

use Jiko\Teamspeak\Com\Teamspeak\TSServerConnections;
...
class BotManager {
  ...
  public function initialize() {
    ...
    $this->ts3DefaultConnection = $this->createServerConnection("God");
    if (is_null($this->ts3DefaultConnection)) {
      echo self::$TAG . "could not initialize bot manager, no teamspeak server connection!\n";
      return false;
    }
    return true;
  }
  ...
  public function update() {
    ...
    $tpoll = (getenv("TS3_POLLINTERVAL") * 1000.0);
  ...
  public function notifyBotMessage($botType, $botId, $text) {
    ...
    if ($bot) {
      try {
        ...
        error_log(self::$TAG, "error occured while processing received message in bot: " . $bot->getName());
        error_log(self::$TAG, " backtrace\n" . $e->getTraceAsString());
      }
      return true;
    }
    return false;
